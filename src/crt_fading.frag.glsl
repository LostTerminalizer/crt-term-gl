...
void main() {
    ...
    vec3 bg = lerp(clamp(sample(uv).rgb - fade, 0.0, 1.0), vec3(0.0), vec3(fade));

    vec2 pixel = pixelSize * 1;

    vec3 blur = vec3(0.0);
    blur = max(blur, sample(uv + vec2(0, -pixel.y)).rgb);
    blur = max(blur, sample(uv + vec2(0, pixel.y) ).rgb);
    blur = max(blur, sample(uv + vec2(pixel.x, 0) ).rgb);
    blur = max(blur, sample(uv + vec2(-pixel.x, 0)).rgb);

    blur = blur * 0.1;// + rand(uv) * 0.05;

    bg = clamp(bg + blur, 0.0, 1.0);
